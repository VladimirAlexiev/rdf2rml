...
- [[#todo-tasks][ToDo Tasks]]
  ...
  - [[#mid-term][Mid-Term]]
    ...
    - [[#display-named-graphs][Display Named Graphs]]
...
- Hossein Rimaz (2025). [[https://aas.metaphacts.cloud/conversion/][AAS Conversion]] at Metaphacts, [[https://hub.docker.com/repository/docker/mhrimaz/aas-rdf-util/general][aas-rdf-util]] on DockerHub.
  Graphwise hopes to collaborate with Metaphacts on semantic representation, storage and querying of Asset Administration Shell data.

[[./gallery/AAS-Convertor.png][AAS-Convertor.png]]
...
Now it only supports Turtle and Trig 1.0, because it concatenates ~prefixes.ttl~ to the main file.
...
The best way is to have a smarter script or ~Makefile~ that uses the following [[http://plantuml.com/command-line][plantuml command-line]] features:
...
- Use [[https://plantuml.com/command-line#ce21470ab49d1d19][-checkmetadata]] to skip ~png~ files that don't need to be regenerated.
...
[[https://github.com/vladimiralexiev/rdf2rml/issues/36][#36 upgrade to Attean]] upgrade to Attean for two reasons:
- it's faster
- has trig 1.1 support

*** Integrate in Emacs ~org-mode~
Write Turtle, see diagram (easy to do)

*** Display Named Graphs
[[https://github.com/vladimiralexiev/rdf2rml/issues/52][#52 support named graphs]].
- This is useful since ~rdf2sparql~ already supports named graphs (CONSTRUCT GRAPH) with SparqlAnything
- The consensus is to use different color per graph. Example:

[[./doc/img/attr-sysRefPref-with-graph-colors.png]]
