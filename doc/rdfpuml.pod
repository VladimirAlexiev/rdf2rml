...
=item * If a node is inlined, its label is added as a comment. If several nodes are inlined at the same property, all their comments are also gathered together.

=item * If a node is shown normally, its label is shown amongst the other node attributes.

=item * C<puml:label> is used to give a label without any predicate (attribute) name. It's printed in red (but only if single-line). It's used with completely different meanings in B<rdf2rml> (to specify a SQL table or join) and in B<rdf2sparql> (to specify a row filter): I know this is a hack.
...
The tiny one in the middle has no attributes whatsoever, only the C<rdf:first, rdf:next> outgoing links.
It should have had a type C<rdf:List>, this is an omission in the example.
...
=head2 PlantUML Options

You can pass options and pragmas to PlantUML using the C<puml:options> property (attached to an empty node).
The default options are as per F<plantuml.cfg>:
...
You can use C<left to right direction> to fit diagrams with large nodes (see two examples below).
Don't forget to add the C<hide> options, else you'll get unwanted compartments and circles in nodes:
...
You can also try your luck with L<smetana|https://plantuml.com/smetana02>, which uses an internal Java implementation of GraphViz instead of an external C program.
One way to invoke C<smetana> is by adding a pragma to C<puml:options>.
See F<text/saref4city> for another way, and a trial.

    !pragma layout smetana

First example (F<test/saref4city>):

=for html
<img src="../test/saref4city/example1-saref4city.png">

=for text
See F<../test/saref4city/example1-saref4city.png>

Second example (F<test/permid>):

=for html
<img src="../test/permid/sample-RobinSmith-positions.png">

=for text
See F<../test/permid/sample-RobinSmith-positions.png">

=head2 Unicode

The input Turtle can include Unicode chars (accented chars, Cyrillic, etc).
Please invoke the script with option C<-C> for proper processing of Unicode, eg:

  perl -C -S rdfpuml.pl file.ttl

(The option C<-S> causes Perl to search for the script in C<PATH>.)

That's how the included F<bin/rdfpuml.bat> invokes the script.
