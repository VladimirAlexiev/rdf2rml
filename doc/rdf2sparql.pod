...
=item * The B<coalesce()> SPARQL function returns the first of its bound arguments (if any). The extra arguments B<...> in the macro declaration are passed as B<__VA_ARGS__> to that function.

=item * The B<bound()> SPARQL function therefore checks if any of the extra arguments are bound.

=item * If so, the "stringify" macro operator B<#> returns the B<string representation> of the first argument.

=item * Otherwise, the B<?UNDEF> variable is returned, which by convention is never bound.

=item * Be careful about the resulting variable: we need to write out the question mark before it (since the same name B<x> is also used in its stringified form), and we follow it by one underscore B<_> even though the macro name itself starts with underscore.
...
To explain the first couple of lines:
first we check if any of the "business payload" fields are bound, and return the string "address" or UNDEF.
Then we concat this tring with some constant and variabel parts to form a URL.
