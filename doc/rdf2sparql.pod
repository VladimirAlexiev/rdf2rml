...
rdf2sparql - Convert RDF examples to SPARQL queries (OntoRefine or TARQL)
...
=item * L<OntoRefine|https://graphdb.ontotext.com/documentation/standard/loading-data-using-ontorefine.html>, which is an adaptation of OpenRefine for working with RDF data. It exposes a table as a virtual SPARQL endpoint (special service), where each column B<col> of each row is exposed as a variable binding B<?c_col>.
...
We've used OntoRefine for large and complex CSV files,
...
The default is to generate a SPARQL UPDATE query for OntoRefine.
It encloses OntoRefine variables in a B<service> clause that accesses the OntoRefine virtual SPARQL endpoint
...
=item * It overwrites a named graph, so it can be used for ingest or update. The first model line must look like this where B<graph> is the graph to overwrite (in angle brackets).

  ## GRAPH <graph>

=item * You can match real triples in GraphDB, which can be used to resolve entities ("strings" to "things"). A macro name that ends in "URL" (case-insensitively), e.g. B<CB_AGENT_URL(permalink)>, causes the respective patterns to be interpreted outside of the OntoRefine virtual service.
...
=item * OntoRefine or TARQL as described above
...
Process B<model-ontorefine.ru> with OntoRefine:
...
If you save the OntoRefine cleaning script (operations JSON),
you can automate the OntoRefine process by using the
L<ontorefine-client|https://github.com/Ontotext-AD/ontorefine-client> CLI
(it will become part of the OntoRefine distribution).
We have scripts to automate the following steps:

=over

=item * Create an OntoRefine project and capture its project ID

=item * Load tabular data into the OntoRefine project
...
It uses a GRAPH comment to specify which named graph should be overwritten.
...
Next is a very simple model that adds "revenue" into to CrunchBase organizations.

  ## GRAPH <extra/revenue>
  <CB_AGENT_URL(permalink)> ex:annualRevenueUsd "(revenue)"^^xsd:decimal.

The revenue is keyed by "permalink" so we use the B<CB_AGENT_URL()> macro to lookup by permalink.
...
=item * B<CB_AGENT_URL(x)>: uses a real RDF lookup (outside of the OntoRefine virtual endpoint). Before that guards with B<bound(x)> (otherwise an unbound variable will return all agents), then checks for respective B<rdf:type>

=item * B<SPLIT(x)>: uses a "magic predicate" (B<apf:strSplit> in TARQL or B<spif:split> in OntoRefine) to split x and produce multiple bindings
...
=item * The script converts datatyped fields to UPPERCASE, so avoid source fields in all uppercase

=item * To implement B<SPLIT>, use B<apf:strSplit> in TARQL and B<spif:split> in OntoRefine. (In a future version, this tool may take care of that difference.)
...
=item * B<$GRAPH> is the named graph mentioned in the first model line. This way the query can handle both initial data loading and updates. Please note that for Crunchbase it is unfeasible to regenerate all Organization data on every update. So we have a slightly more complex script (not published) that uses a named graph per table B<row> (uuid) not per B<table>, and selects only recently updated rows for processing.

=item * B<PROJECT_ID> is a placeholder that must be replaced with the actual OntoRefine project ID before running the query.

=item * The script has a special case for macro names matching B<*_URL>, that's why the B<CB_AGENT_URL> pattern is evaluated outside of the OntoRefine virtual endpoint.
...
and how to process datatype attachments (which uses variable names converted to uppercase).
...
For OntoRefine, first come silly "aliases" for each variable used in the model
because of some peculiarities of the OntoRefine virtual endpoint
...
As you can see, the B<CB_AGENT_URL> macro is evaluated outside of the OntoRefine virtual endpoint.
